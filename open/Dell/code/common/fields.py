...
class Field:
    ...
    supported_systems: Tuple[System, ...] = (AdHocSystemClassification.CatchAll,)
    ...
    def supports(self, action: Optional[Action], benchmark: Benchmark, scenario: Scenario,
                 system: System, workload_setting: WorkloadSetting) -> bool:
        """
        ...
        return (action is None or action in self.supported_actions) and \
            ...
            any(elem == sys_id for elem in self.supported_systems) and \
...
class Fields(Enum):
    ...
    gpu_batch_size: Field = Field(
        ...
        supported_systems=(AdHocSystemClassification.GPUBased,),
        value_type=int,
        required=True)
    ...
    dla_batch_size: Field = Field(
        ...
        supported_systems=(AdHocSystemClassification.Xavier,),
        required=True)
    ...
    start_from_device: Field = Field(
        ...
        value_type=bool,
        supported_actions=_HARNESS_ACTIONS,
        supported_systems=(AdHocSystemClassification.StartFromDeviceEnabled,))
    ...
    end_on_device: Field = Field(
        ...
        supported_systems=(AdHocSystemClassification.EndOnDeviceEnabled,))
    ...
    numa_config: Field = Field(
        ...
        value_type=bool,
        supported_actions=_HARNESS_ACTIONS)  # TODO: Create an AdHocSystemClassification for multi-GPU systems

    batch_size: Field = Field(
        "batch_size",  # TODO: Rename this to cpu_batch_size
        "Batch size to use for the CPU",
        value_type=int,
        supported_systems=(AdHocSystemClassification.IntelOpenVino,),
        required=True)
...
class MainArgs(Enum):
    ...
    # Xavier power args
    xavier_gpu_freq: Field = Fields.xavier_gpu_freq.value
    xavier_dla_freq: Field = Fields.xavier_dla_freq.value
    xavier_cpu_freq: Field = Fields.xavier_cpu_freq.value
    xavier_emc_freq: Field = Fields.xavier_emc_freq.value
