...
export LD_PRELOAD=/path/to/jemalloc/lib/libjemalloc.so:$LD_PRELOAD # ${WORKDIR}/jemalloc/lib/libjemalloc.so
...
python3 run.py --workload-name ssd-resnet34 \
    ...
    --workload-config ssd-resnet34/pytorch-cpu/scripts/ICX/ssdr34-conig-Offline-ICX40C-2S_acc.json \
    --num-instance 10 \
    --cpus-per-instance 8 \
    --warmup 0 \
    --scenario Offline \
