...
def generate_readme(readme_path, system_name, short_benchmark_name, scenario):
    ...
    For more details, please refer to `closed/Fujitsu/README.md`.""".format(
        benchmark=short_benchmark_name,
        scenario=scenario.valstr()))

    if "HeteroMultiUse" in system_name:
        readme_str = textwrap.dedent("""\
        To run this benchmark, first follow the setup steps in `closed/Fujitsu/README.md`. Then to generate the TensorRT
        engines and run the harness, first read the **Using Multiple MIG slices** section in `closed/Fujitsu/README.md`.
        Then follow the instructions in `closed/Fujitsu/documentation/heterogeneous_mig.md` to run benchmarks.""")
...
def generate_powersetting_adoc(powersetting_path):
    ...
    section of the README.md located in closed/Fujitsu.
    """)
