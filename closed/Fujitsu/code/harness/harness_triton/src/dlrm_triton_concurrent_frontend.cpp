...
size_t DLRM_Triton_SUT::GetNumQuerySamplesToAcquire(size_t num_pairs, int num_successful_tries,
                                                    int num_empty_tries)
{
    return (m_MaxBatchSize - num_pairs) / (0.8 * m_MaxSampleSize);
}
    ...
    while(true)
    {
        ...
        for(size_t idx = 0; idx < m_InputTensors.size(); idx++)
        {
            ...
            for(const auto& sample : req->samples)
            {
                // Get a pointer to the input data
                int8_t* input_data =
                    (int8_t*)qsl->GetSampleAddress(sample.index, idx); // Get address of the query
                ...
                AppendInputDataForAllGPUs(
                    request_block->m_Data, sample.index, std::get<0>(m_InputTensors[idx]).c_str(),
...
bool DLRM_Triton_SUT::CheckContiguity(const std::deque<mlperf::QuerySample>& samples)
{
    return false;
}
