...
@ConfigRegistry.register(HarnessType.Custom, AccuracyTarget.k_99, PowerSetting.MaxP)
class A30x10(BenchmarkConfiguration):
    system = System("A30", Architecture.Ampere, 10)
    coalesced_tensor = True
    enable_interleaved_top_mlp = False
    gpu_copy_streams = 1
    gpu_inference_streams = 1
    ...
    num_staging_batches = 8
    num_staging_threads = 8
    server_target_qps = 1020000
    use_jemalloc = False
    use_small_tile_gemm_plugin = True
    numa_config = "0-4:0-4,76-80&5-9:38-42,114-118"
    scenario = Scenario.Server
    benchmark = Benchmark.DLRM
...
@ConfigRegistry.register(HarnessType.Custom, AccuracyTarget.k_99_9, PowerSetting.MaxP)
class A30x10_HighAccuracy(BenchmarkConfiguration):
    system = System("A30", Architecture.Ampere, 10)
    coalesced_tensor = True
    enable_interleaved_top_mlp = False
    gpu_copy_streams = 1
