...
CPU_SOCKETS = re.findall(r'Socket\(s\)\:\s+(\d+)',stdout)
...
class SystemClass(object):
    ...
    def get_match(self, name, count, pci_id=None, mig_conf=None):
        ...
        if pci_id is not None and len(self.pci_ids) > 0:
            #if socket >= 2, that's most of Dell's systems so stick with normal match
            if int(CPU_SOCKETS) >= 2:
                gpu_match = pci_id in self.pci_ids
            #if socket == 1, Dell is only submitting on one of these so it can't be anything else, just hard-code that match
            else:
                if self.gpu == "XR12_A10":
                    gpu_match = pci_id in self.pci_ids
        # Attempt to match a name if PCI ID is not specified, or if the system has no known PCI IDs
        # This has been converted to an if block by Dell, but we explicitly show condition for clarity
        # Dell should only have one system that fits this, which is the VMWare submission, which leaves the self.pci_id field intentionally blank to trigger this
        if pci_id is None or len(self.pci_ids) == 0:
...
class KnownSystems(object):
    ...
    DSS8440_A30 = SystemClass("DSS8440_A30", ["A30"], ["20B7"], Architecture.Ampere, [8],
    ...
    R750xa_A100_PCIE_80GB = SystemClass("R750xa_A100-PCIE-80GB", ["A100-PCIE-80GB"], ["20B5"], Architecture.Ampere, [4],
    ...
    R7525_A30 = SystemClass("R7525_A30", ["A30"], ["20B7"], Architecture.Ampere, [3])
    XE2420_A10 = SystemClass("XE2420_A10", ["A10"], ["2236"], Architecture.Ampere, [1, 2])
    XE8545_A100_SXM_80GB = SystemClass("XE8545_A100-SXM-80GB", ["A100-SXM-80GB"], ["20B2"], Architecture.Ampere, [4],
                                valid_mig_slices=[MIGSlice(1, 10), MIGSlice(2, 20), MIGSlice(3, 40)])
    XR12_A10 = SystemClass("XR12_A10", ["A10"], ["2236"], Architecture.Ampere, [2])
    R7525_vA100_PCIE_40GB = SystemClass("R7525_vA100-PCIE-40GB", ["GRID A100-40C"], [], Architecture.Ampere, [3])
