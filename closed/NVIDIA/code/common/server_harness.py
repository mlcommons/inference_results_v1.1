...
class TritonHarness(BaseBenchmarkHarness):
    ...
    def _build_custom_flags(self, flag_dict):
        ...
        flag_dict["use_concurrent_harness"] = self.args.get("use_concurrent_harness", False)
        flag_dict["num_batchers"] = self.args.get("num_concurrent_batchers", 1)
        flag_dict["num_issuers"] = self.args.get("num_concurrent_issuers", 1)

        if self.has_dla and self.name != Benchmark.SSDMobileNet:
            flag_dict["use_dla"] = True
            flag_dict["use_concurrent_harness"] = True  # DLA Needs concurrent harness
    ...
    def setup_triton_model_repo(self, engine_info):
        ...
        if "output_pinned_memory" in self.args:
            ...
            config["output_pinned"] = "false" if self.scenario == Scenario.SingleStream else "true"
        config["gather_kernel_buffer_threshold"] = self.args.get("gather_kernel_buffer_threshold", 0)
        config["busy_wait_events"] = "true" if self.scenario == Scenario.SingleStream else "false"
