...
class ConfigRegistry(object):
    ...
    def _check_harness_support(cls, config, workload_setting):
        ...
        if harness_type == HarnessType.HeteroMIG:
            if config.system.mig_conf is None or config.system.gpu not in ["A100-SXM-80GB", "A30"]:
                raise ConfigInvalidHarnessTypeError(config.__name__, harness_type)
        if harness_type == HarnessType.Triton:
            # ARM Server does not currently support Triton
            if config.system.cpu_arch == CPUArch.aarch64 and config.system.arch != Architecture.Xavier:
                raise ConfigInvalidHarnessTypeError(config.__name__, harness_type)
