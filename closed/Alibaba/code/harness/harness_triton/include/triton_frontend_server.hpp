...
class Triton_Server_SUT : public mlperf::SystemUnderTest
{
  public:
    ...
    Triton_Server_SUT(std::string name, std::string model_repo_path, std::string model_name,
                      uint32_t model_version, bool use_dlrm_qsl, bool start_from_device,
                      bool pinned_input, uint64_t request_count)
        : m_Name(name), m_ModelRepositoryPath(model_repo_path), m_ModelName(model_name),
          ...
          m_StartFromDevice(start_from_device), m_RequestCount(request_count)
    {
