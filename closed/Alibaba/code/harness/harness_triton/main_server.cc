...
void doInference()
{
    ...
    triton_frontend::ServerSUTPtr_t sut = std::make_shared<triton_frontend::Server_SUT>(
        ...
        FLAGS_use_dlrm_qsl, FLAGS_start_from_device, FLAGS_pinned_input, FLAGS_request_pool_count);

    if(FLAGS_use_dlrm_qsl)
    {
        sut->Init(FLAGS_min_sample_size, FLAGS_max_sample_size, FLAGS_buffer_manager_thread_count, FLAGS_batch_triton_requests, FLAGS_check_contiguity, FLAGS_numa_config);
    }
    else
    {
        sut->Init(1, 1, FLAGS_buffer_manager_thread_count, FLAGS_batch_triton_requests, FLAGS_check_contiguity, FLAGS_numa_config);
    }
    sut->ModelMetadata();
