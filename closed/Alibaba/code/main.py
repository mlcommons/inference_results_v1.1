...
from code.common.scopedPowerLimit import ScopedPowerLimit
...
def copy_engine(benchmark, source_engine_setting):
    ...
    logging.info("Copying {:} to {:}".format(old_path, new_path))
...
def dispatch_action(main_args, benchmark_conf, conf_ver, equiv_engine_setting=None):
    ...
    if benchmark_conf["use_cpu"]:
        if main_args.get("power_limit", None) is not None:
            benchmark_conf["power_limit"] = main_args["power_limit"]
        power_limit = benchmark_conf.get("power_limit", None)
    else:
        power_limit = None

    # Generate engines.
    if action == "generate_engines":
        with ScopedPowerLimit(power_limit):
            ...
            if not main_args["no_child_process"]:
                if benchmark_conf["scenario"] == SCENARIOS.Server and active_sms is not None and active_sms < 100:
    ...
    for benchmark in benchmarks:
        for scenario in scenarios:
            ...
            for workload_setting in workload_settings:
                ...
                config_dict["config_name"] = f"{system_id}_{str(benchmark.value)}_{str(scenario.value)}"
                ...
                config_dict["inference_server"] = str(workload_setting.harness_type)

                # Use strings instead of the Enums for now until the rest of the codebase is refactored.
                config_dict["system_id"] = system_id
                config_dict["benchmark"] = str(benchmark.value)
                config_dict["scenario"] = str(scenario.value)
