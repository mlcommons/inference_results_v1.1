...
- LWIS and other "custom" harnesses (i.e. `harness_dlrm`, `harness_bert`, and `harness_rnnt`) require users to specify the `numa_config` key in the system's `BenchmarkConfiguration` in the appropriate Python file. The value of this key is specified as a string with the following convention:
...
- The normal Triton harness currently does not have a NUMA aware setting.
