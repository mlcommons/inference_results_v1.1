                                   ...
                                   bool check_contiguity, const std::string& numa_config_str)
{
    ...
    if(!m_BatchTritonRequests)
    {
        ...
        LOG(INFO) << "Allocated Pinned memory pool of "
                  << pool_item_count_per_instance * pool_item_size << " bytes for every instance.";
    }
...
void Concurrent_Frontend_SUT::Warmup(double duration_sec, double expected_qps)
{
    ...
    /* Calculate the number of inferences to send
       An "inference" can either be a single sample or a batch, depending on
       BatchTritonRequests.
    ...
    */
    ...
    while(inferences_sent < num_inferences)
    {
        ...
        for(size_t idx = 0; idx < m_InputTensors.size(); idx++)
        {
            ...
            AppendInputDataForAllGPUs(request_block->m_Data, samples[0],
                                      std::get<0>(m_InputTensors[idx]).c_str(), input_size, idx);
            AppendInputDataForAllQSLs(request_block->m_Data, samples[0],
                                      std::get<0>(m_InputTensors[idx]).c_str(), input_size, idx);
        }
