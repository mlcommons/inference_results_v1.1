...
from code.common import is_aarch64, is_xavier, logging, dict_get
...
class SSDMobileNet(TensorRTEngineBuilder):
    ...
    def initialize(self):
        ...
        # Load timing cache to skip the missing XMMA first_layer kernel for aarch64 SM80
        if is_aarch64() and not is_xavier():
            timing_cache_file = 'code/ssd-mobilenet/tensorrt/aarch64-timing.cache'
            with open(timing_cache_file, 'rb') as f:
                buf = f.read()
                timing_cache = self.builder_config.create_timing_cache(buf)
                self.builder_config.set_timing_cache(timing_cache, True)
