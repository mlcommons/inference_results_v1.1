# MLPerf Inference v1.1 Optimized Implementations
...
Our submission implements several inference harnesses stored under closed/Inspur/code/harness:
...
- You, Inspur, or MLCommons has decided that something in the preprocessing step needed to be altered
...
- A100-SXM-80GBx8 (Inspur NF5488A5 A100, 80GB variant)
- A100-SXM-80GBx8 (Inspur NF5688M6 A100, 80GB variant)

Edge Systems:

- A100-SXM-80GBx1
- A100-PCIex2 (Inspur NE5260M5, 40 GB variant)
...
First, create your SystemClass. You will have to add your system description to closed/Inspur/code/common/system_list.py. At the bottom of the file, there is a class called 'KnownSystems'. This class defines a list of 'SystemClass' objects that describe supported systems as follows:
...
4. architecture: There is an 'Architecture' Enum at the top of the file. Use one of those fields. Example: Architecture.Ampere
...
- The docker container does not copy the files, and instead **mounts** the working directory (closed/Inspur) under /work in the container. This means you can edit files outside the container, and the changes will be reflected inside as well.
...
Inspur's power category submission is called 'MaxQ'. To run the harness with power measurements, follow the steps below:

1. Set the machine to the desired power mode for our MaxQ submissions.
2. Set up a Windows power director machine with the following requirements.
   ...
   4. There exists an administrator user `lab` with password `labuser`. If your administrator account has different login information, set `POWER_SERVER_USERNAME` and `POWER_SERVER_PASSWORD` in `closed/Inspur/Makefile` to the correct credentials.
   5. OpenSSH server is installed and enabled on the machine, listening on port 22.
3. Set the power meter configuration in `power/server-ne5260m5.cfg`.
...
5. In Inspur's submission, we use the Yokogawa WT310E meter..
...
See documentation/calibration.md for an explanation on how calibration is used for Inspur's submission.
