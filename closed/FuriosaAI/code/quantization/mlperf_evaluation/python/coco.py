...
# TODO model function will be imported from onnx-model-exporter directly
sys.path.append(os.path.dirname(os.path.dirname(__file__)))
from tmp.ssd_mobilenet_v1 import MLCommons_SSDMobileNetV1 as ssd_mobilenet_pt
    ...
    def finalize(self, result_dict, ds=None, output_dir=None):
        ...
        for batch in range(0, len(self.results)):
            ...
            for idx in range(0, len(self.results[batch])):
                ...
                # assure torch.tensor to be placed on host cpu memory
                detections.append(
                    np.array(
                        [
                            tensor.cpu() if torch.is_tensor(tensor) else tensor
                            for tensor in detection
                        ]
                    )
                )
...
class PostProcessCocoONNXPP(PostProcessCocoTf):
    """
    Post processing required by ssd-resnet34 / pytorch
    """
    ...
    def __init__(self):
        super().__init__()
        self.use_inv_map = True
        # logging only 3:Error and 4:Fatal
        ort.set_default_logger_severity(3)
        path = f"{os.path.dirname(__file__)}/mlcommons_ssd_resnet34_postprocess.onnx"
    ...
    def __call__(self, results, ids, expected=None, result_dict=None):
        ...
        for idx, log in enumerate(logits):
            feed_dict[f"ssd1200/multibox_head/cls_{idx}/BiasAdd:0"] = log
        for idx, loc in enumerate(locations):
            feed_dict[f"ssd1200/multibox_head/loc_{idx}/BiasAdd:0"] = loc

        bboxes, scores, classes = self.sess.run([], input_feed=feed_dict)
        processed_results = super(PostProcessCocoONNXPP, self).__call__(
            [bboxes, classes, scores], ids, expected, result_dict
        )

        return processed_results


class PostProcessCocoSSDMobileNetORT(PostProcessCocoPt):
    """
    Post processing required by ssd-mobilenet / onnxruntime
    """

    def __init__(self, use_inv_map, score_threshold):
        super(PostProcessCocoSSDMobileNetORT, self).__init__(use_inv_map, score_threshold)
        self.use_inv_map = use_inv_map
        self.score_threshold = score_threshold
    ...
    def __call__(self, results, ids, expected=None, result_dict=None):
        ...
        logits = [torch.from_numpy(log) for log in results[:6]]
        locations = [torch.from_numpy(loc) for loc in results[6:]]
        results = ssd_mobilenet_pt().postprocess(logits, locations)
        processed_results = super(PostProcessCocoSSDMobileNetORT, self).__call__(
...
from tmp.ssd_resnet34_postprocess import encoder


class PostProcessCocoONNXNP(PostProcessCocoTf):
    """
    ...
    def __call__(self, results, ids, expected=None, result_dict=None):
        ...
        processed_results = super(PostProcessCocoONNXNP, self).__call__(
            results, ids, expected, result_dict
        )
        return processed_results
