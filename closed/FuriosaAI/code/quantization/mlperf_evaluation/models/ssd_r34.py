    ...
    def _build_additional_features(self, input_channels):
        ...
        self.additional_blocks.append(
            nn.Sequential(
                ...
                nn.Conv2d(
                    256, input_channels[idx + 1], kernel_size=3, padding=1, stride=self.strides[2]
                ),
                nn.ReLU(inplace=True),
            )
        )
        ...
        self.additional_blocks.append(
            nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    256, input_channels[idx + 1], kernel_size=3, padding=1, stride=self.strides[3]
                ),
                nn.ReLU(inplace=True),
            )
        )
        ...
        self.additional_blocks.append(
            nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    128, input_channels[idx + 1], kernel_size=3, padding=1, stride=self.strides[4]
                ),
                nn.ReLU(inplace=True),
            )
        )
