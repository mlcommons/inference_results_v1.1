...
class ONNXCalibrator:
    ...
    def calibrate_with_data_loader(self, dataloader: DataLoader) -> Dict[str, Tuple[float, float]]:
        ...
        dataset = map(
            lambda data, _: {self.input_tensors[0][0]: data.detach().cpu().numpy()}, dataloader
        )
        return calibrate(self.model, dataset)

    def calibrate(self, dataset: Iterable[Dict[str, np.ndarray]]) -> Dict[str, Tuple[float, float]]:
    ...
    def calibrate_with_random(
        ...
        return calibrate(
            self.model,
            map(lambda _: populate_random_data(self.input_tensors), range(num_data or 10)),
        )
